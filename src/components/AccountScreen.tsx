...
export default function AccountScreen({ lang, profile, onBack, onLogout, onLangToggle, onProfileUpdated, onOpenSubscription }: Props) {
  ...
  const handleAvatarUpload = async (rawFile: File) => {
    ...
    try {
      ...
      setAvatarError(e instanceof UploadValidationError ? e.message : tr('រូបភាពមិនត្រឹមត្រូវ', 'Invalid image'));
      return;
    }
  ...
  const handleQrUpload = async (rawFile: File) => {
    ...
    try {
      ...
      setQrError(e instanceof UploadValidationError ? e.message : tr('រូបភាពមិនត្រឹមត្រូវ', 'Invalid image'));
      return;
    }
    const ext = file.name.split('.').pop() || 'png';
