...
export default function InvoiceScreen({ lang, profile, onBack, editInvoiceId }: Props) {
  ...
  const handleQRUpload = async (rawFile: File) => {
    ...
    try {
      ...
      setQrUploadError(e instanceof UploadValidationError ? e.message : tr('រូបភាពមិនត្រឹមត្រូវ', 'Invalid image'));
      return;
    }
    const { data: userData } = await supabase.auth.getUser();
    if (!userData.user) {
    ...
    if (uploadError) {
      setQrUploadBusy(false);
      setQrUploadError(uploadError.message);
      return;
    }
    const { data: pubData } = supabase.storage.from('qr-codes').getPublicUrl(path);
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ qr_code_url: pubData.publicUrl })
      .eq('id', userData.user.id);
    if (updateError) {
      setQrUploadBusy(false);
      setQrUploadError(updateError.message);
      return;
    }
